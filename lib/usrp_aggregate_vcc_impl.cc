        ...
        int usrp_aggregate_vcc_impl::general_work(int noutput_items,
            ...
            if (tagVector.size() > 0) {
                for (int i = 0; i < (int)tagVector.size(); i++) {
                    ...
                    bool freq_changed = pmt::eq(
                            pmt::dict_ref(d_FFT_tune_info[d_FFT_which], pmt::intern("rx_freq"), pmt::from_double(-2)),
                            pmt::dict_ref(tagval, pmt::intern("rx_freq"), pmt::from_double(-2)));
                    bool rate_changed = pmt::eq(
                            pmt::dict_ref(d_FFT_tune_info[d_FFT_which], pmt::intern("rx_rate"), pmt::from_double(-2)),
                            pmt::dict_ref(tagval, pmt::intern("rx_rate"), pmt::from_double(-2)));
            ...
            if ((toc - d_tic) > d_interval) {
                ...
                new_tag = pmt::dict_add(new_tag, pmt::intern("rx_freq"),
                                        pmt::dict_ref(d_FFT_tune_info[selected], pmt::intern("rx_freq"), pmt::from_double(-1)));
                new_tag = pmt::dict_add(new_tag, pmt::intern("rx_rate"),
                                        pmt::dict_ref(d_FFT_tune_info[selected], pmt::intern("rx_rate"), pmt::from_double(-1)));
                if (d_last_FreqSrate[0] != pmt::to_double(pmt::dict_ref(new_tag, pmt::intern("rx_freq"), pmt::from_double(-1))) ||
                    d_last_FreqSrate[1] != pmt::to_double(pmt::dict_ref(new_tag, pmt::intern("rx_rate"), pmt::from_double(-1)))) {
                    add_item_tag(0, nitems_written(0), pmt::intern("tune"), new_tag);
                    d_last_FreqSrate[0] = pmt::to_double(pmt::dict_ref(new_tag, pmt::intern("rx_freq"), pmt::from_double(-1)));
                    d_last_FreqSrate[1] = pmt::to_double(pmt::dict_ref(new_tag, pmt::intern("rx_rate"), pmt::from_double(-1)));
                    std::cout << "USRPAggregate: new freq/srate: " << d_last_FreqSrate[0] << " " << d_last_FreqSrate[1] << std::endl;
                }
